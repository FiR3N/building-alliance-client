...
const ReviewsList: FC<ReviewsListProps> = ({ isAdmin }) => {
  const [page, setPage] = useState<number>(1);
  const [limit] = useState<number>(9);
  const [isReviewsModalActive, setIsReviewsModalActive] =
    useState<boolean>(false);

  let { error, data: reviewsList } = isAdmin
    ? reviewsAPI.useGetReviewsQuery({
        limit,
        page,
        isNotOnlyPublished: true,
      })
    : reviewsAPI.useGetReviewsQuery({
        limit,
        page,
        ...
        {!isAdmin && (
          <div className={cls.reviewsListTitle}>
            ...
            <p className="default-text">
              ...
              результатами. Мы гордимся нашей репутацией и стараемся
              предоставлять высококачественные услуги, а отзывы наших клиентов
              помогают нам улучшать и развивать нашу деятельность.{" "}
            </p>
          </div>
        )}
